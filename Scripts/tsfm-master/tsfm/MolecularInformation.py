...
class DistanceCalculator:
    ...
    def get_distance(self, ResultsDict):
        ...
        Note:
            ...
            passed to the distance method set when?????? with the :class:`DistanceCalculator`
            ...
            for coord in ResultsDict[result].basepairs:
                if (coord in ResultsDict[result].info):
                    for pairtype in ResultsDict[result].info[coord]:
                        self.featureSet.add("{}{}".format("".join(str(i) for i in coord),
                                                          pairtype))  # ?????????????????????????? what is this
                    ...
                    for base in [nuc for nuc in ResultsDict[result].inverseInfo[coord] if not nuc == "-"]:
                        ...
                        for function in ResultsDict[result].inverseHeight[coord][base]:
                            pandasDict[result].loc[row, function] = ResultsDict[result].inverseHeight[coord][base][
                                function]
            # c=0
            # for i, row in pandasDict[result].iterrows():
            #
            #     if pandasDict[result].loc[i,].drop('bits').sum() < 1:
            #        c=i
            #        print(pandasDict[result].loc[i,],"pandasDict[result].loc[i,]")
            #        print(pandasDict[result].loc[i,].drop('bits').sum(),"summmm")
            #        # print(pandasDict[result].loc[i,], "loc[row, function")
            #        # print(pandasDict[result].loc[i,].drop('bits').sum(), "pandasDict[result].loc[row,].sum()")
        ...
        for frame in pandasDict:
            ...
            # temp = pandasDict[frame]['bits']
            # #print(temp,"temp")
            # pandasDict[frame] = pandasDict[frame].drop('bits', axis=1).div(
            #     pandasDict[frame].drop('bits', axis=1).sum(axis=1), axis=0)
            # pandasDict[frame]['bits'] = temp
            # #print(temp, "temp")
            # for i, row in pandasDict[frame].iterrows():
            #     if row['bits'] == 0:
            #         print(row, "heh")
    ...
    def rJSD(self, pandasDict):
        ...
        for pair in pairwise_combinations:
            distance = 0
            # print(pair, "pairppppppppppppppppppppppp")
            for i, row in pandasDict[pair[0]].iterrows():
                # print(row['bits'],"row['bits'])))))))))))))))))))))))))))))))))))")
                # print(pandasDict[pair[1]].loc[i, 'bits'],"pandasDict[pair[1]].loc[i, 'bits'])))))))))))))))")
...
class FunctionLogo:
    ...
    def calculate_exact(self, n, proc, inverse=False):
        ...
        if (inverse):
            ...
            for x in exact_results:
                ...
                # exact is an array of length n = 10
    ...
    def calculate_kld(self, types, back_prior, fore_prior, back_post, fore_post, pairs, basepairs, singles):
        """
