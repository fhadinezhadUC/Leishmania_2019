...
import sys
import os
import MolecularInformation as MolecularInformation
...
def main():
    ...
    # parser.add_argument("--logo2", help='id logo', action="store_true")
    ...
    if (args.file):
        ...
        if (args.logo and not args.inverse):
            ...
            for key in results:
                # results[key].add_information(info=IdInfoH, height=idHeightH)
                print("Writing function logo postscript files for {}".format(key))
    ...
    if args.IDlogo & args.KLDlogo & args.bt:
        logo_dict[key_1].func_ID_KLD_2table(fore_logo_info=info_height_dic[key_1]['info'],
                                            ...
                                            kld_info=kld_info1,
                                            kld_height=kld_height1, functions=types, states=single, pos=pos, fore=key_1)
        ...
        logo_dict[key_2].func_ID_KLD_2table(fore_logo_info=info_height_dic[key_2]['info'],
                                            ...
                                            kld_info=kld_info2,
                                            kld_height=kld_height2, functions=types, states=single, pos=pos, fore=key_2)
