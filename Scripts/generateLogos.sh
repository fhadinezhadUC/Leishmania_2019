...
python3 "$tsfmpath/tsfm.py" -c "$folderpath/TryTrypHomoC_struct_file.txt" --logo "$folderpath/$name/clustalW/$name"
mv -- *.eps "$folderpath/Logos/$name/Func"

python3 "$tsfmpath/tsfm.py" -c "$folderpath/TryTrypHomoC_struct_file.txt" --IDlogo "$folderpath/$name/clustalW/$name" "$folderpath/HOMO/clustalW/HOMO"
mv -- *.eps "$folderpath/Logos/$name/ID"

python3 "$tsfmpath/tsfm.py" -c "$folderpath/TryTrypHomoC_struct_file.txt" --KLDlogo "$folderpath/$name/clustalW/$name" "$folderpath/HOMO/clustalW/HOMO"
mv -- *.eps "$folderpath/Logos/$name/KLD"

python3 "$tsfmpath/tsfm.py" -c "$folderpath/TryTrypHomoC_struct_file.txt" --IDlogo --KLDlogo --bt "$folderpath/$name/clustalW/$name" "$folderpath/HOMO/clustalW/HOMO"
rm -- *.eps
