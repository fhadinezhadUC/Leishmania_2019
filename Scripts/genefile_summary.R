genefile.summary <- function() {
  ...
  genefilepath <-
    "/home/fatemeh/Leishmania_2019/Leishmania_2019/Results/Integrated_Genes/integrated_tse_ara2.txt"
  geneDF <-
    read.table(genefilepath, header = TRUE, colClasses = "character")
  vidualization2(geneDF, outputpath)
}
vidualization2 <- function(integrated_tse_ara, outputpath) {
  genometRNAdf <-
    data.frame(
      integrated_tse_ara$sourceOrg,
      integrated_tse_ara$Func,
      integrated_tse_ara$foundby
    )
  names(genometRNAdf) <- c("sourceOrg", "func", "foundby")
  genome_list = split(genometRNAdf, f = genometRNAdf$sourceOrg)
  
  tRNAcountdf <- data.frame(names(genome_list))
  tRNAcountdf$counts <- 0
  
  names(tRNAcountdf) <- c("genome", "counts")
  ...
  for (i in 1:length(genome_list)) {
    ...
    tRNAcountdf$counts[i] <- nrow(intersect_tse_ara)
  }
  tRNAcountdf <- tRNAcountdf[order(tRNAcountdf$counts), ]
  tRNAcountdf$genome <-
    factor(tRNAcountdf$genome, levels = tRNAcountdf$genome)
  
  library(ggplot2)
  p <- ggplot(data = tRNAcountdf, aes(x = genome, y = counts)) +
  ...
  p
  ggsave(
    paste(outputpath, "intersecttRNAcounts2.png", sep = ""),
    width = 14,
    height = 7
  )
  ...
  for (i in 1:length(genome_list)) {
    ...
    intersect_tse_ara <- currentgenedf[both, ]
    countsdf <-
      as.data.frame(table(as.character(intersect_tse_ara$func)))
  ...
  p
  ggsave(
    paste(outputpath, "ara_funcPerc2.png", sep = ""),
    width = 14,
    height = 7
  )
  ...
  inter_genes <-
    integrated_tse_ara[integrated_tse_ara$foundby == "both", ]
  inter_diff_type <-
    inter_genes[inter_genes$arafunc != inter_genes$tsefunc, ]
...
vidualization <- function(integrated_tse_ara, outputpath) {
  ...
  names(genometRNAdf) <-
    c("sourceOrg", "arafunc", "tsefunc", "foundby")
  genome_list = split(genometRNAdf, f = genometRNAdf$sourceOrg)
  ...
  for (i in 1:length(genome_list)) {
    ...
    intersect_tse_ara <-
      intersect_tse_ara[as.character(intersect_tse_ara$tsefunc) == as.character(intersect_tse_ara$arafunc), ]
  ...
  library(ggplot2)
  p <-
    ggplot(data = tRNAcountdf, aes(x = genome, y = interscounts)) +
    geom_bar(stat = "identity", fill = "steelblue") + geom_text(
  ...
  p
  ggsave(
    paste(outputpath, "ara_funcPerc.png", sep = ""),
    width = 14,
    height = 7
  )
...
summery_table <- function(integrated_tse_ara, outputpath) {
  ...
  intersect_tse_ara <-
    intersect_tse_ara[intersect_tse_ara$tsefunc == intersect_tse_ara$arafunc, ]
  ...
  for (i in 6:ncol(summerytable)) {
    summerytable[, i] <-
      format(as.numeric(summerytable[, i]), digits = 4)
  }
  summerytable[, 4] <-
...
endDisplacement <- function(integrated_tse_ara, outputpath) {
  ...
  ggsave(
    paste(outputpath, "EndDisplacement.png", sep = ""),
    width = 14,
    height = 7
  )
}
